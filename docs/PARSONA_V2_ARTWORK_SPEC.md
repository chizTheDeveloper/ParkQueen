...
## DEV-only provisional layered MVP checkpoint
...
- Provisional technical delivery: **15 WebP runtime layers and 15 matching PNG masters**: one opaque background, eight hair layers, four top layers, and two accessory layers.
- Generator: `scripts/build-parsona-v2-mvp.mjs`.
- Runtime paths remain the exact final manifest paths. Future art refinements must not change an ID, path, saved-data shape, or compositor order.
- These files are a fallback technical checkpoint, not final production art. Their manifest entries remain `pending`, and the DEV-only MVP resolver cannot make them production-approved.
