...
| TM-16 | Mutable CI action references | MEDIUM | Checkout/setup actions use version tags; Gitleaks action is SHA-pinned | **Partially fixed** (commit `93aa02b`, updated in `c44d3b7`+): `permissions: contents: read` and `timeout-minutes` on both workflows; `actions/checkout` pinned to `11bd71901bbe5b1630ceea73d27597364c9af683` (v4.2.2); `setup-node` and `setup-java` still use version tags — SHA lookup commands documented inline in the workflow files: `gh api repos/actions/setup-node/git/ref/tags/v4 --jq .object.sha` |
