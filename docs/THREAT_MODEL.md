...
| TM-12 | App Check not yet enforced | HIGH | Source now prepared: `initializeAppCheck` with `ReCaptchaEnterpriseProvider` added to `firebaseConfig.ts`, gated on `VITE_FIREBASE_APPCHECK_SITE_KEY`; debug-token path DEV-only. Provider not yet registered; enforcement not enabled; public clients can still directly exercise Auth, Firestore, Storage, and callable quotas until enforcement is activated | **SOURCE PREPARED — PROVIDER REGISTRATION, MONITORING, AND ENFORCEMENT PENDING**. To complete: (1) Register app in Firebase Console → App Check → Apps; (2) Add `VITE_FIREBASE_APPCHECK_SITE_KEY=<site-key>` to `.env.production`; (3) Deploy and monitor Firebase Console → App Check → Metrics; (4) Enable `enforceAppCheck: true` on callables per `docs/APP_CHECK_ROLLOUT.md`. Native packaging will require `AppAttestProvider` (iOS) and `PlayIntegrityProvider` (Android) instead of reCAPTCHA. |
...
| TM-14 | Admin bootstrap exposure | HIGH | `bootstrapAdmin` is remotely callable; check-then-set race allowed two concurrent calls to both bootstrap admin | **Fixed in source** (commit `93aa02b`): Firestore transaction on `adminBootstrap/singleton` sentinel replaces query-based check; second concurrent call fails inside the transaction; `adminBootstrap` collection is write-blocked in Rules; 3 emulator tests added. **Residual risk (post-transaction claim failure):** if `setCustomUserClaims` throws after the transaction commits, the singleton doc exists but the user's Auth token carries no `admin: true` claim. Probability: very low (Admin SDK `setCustomUserClaims` is reliable). **Recovery:** (1) In Firebase Console → Authentication, confirm the user UID from `adminBootstrap/singleton`; (2) run `admin.auth().setCustomUserClaims(uid, { admin: true })` via Cloud Shell or a one-off Admin SDK script; (3) user must sign out and back in to obtain a fresh token with the claim. No code change needed; Firestore singleton is intact and prevents re-bootstrap. |
...
| TM-17 | Sensitive logging | MEDIUM/HIGH | Functions log street candidate keys/cross streets and some IDs; browser logs may expose operational errors | **CLOSED**: (1) `redactForLog()` and `sanitizeError()` utilities exist in both `functions/redactForLog.js` (CJS, used in Cloud Functions) and `utils/redactForLog.ts` (ESM, used in client/tests) — SENSITIVE regex scrubs phone, email, token, fcm, lat, lng, coordinate, message, body, text, content, prompt, response, reply, password, secret, credential, apikey, auth, otp, code; (2) all `console.error` call sites in `functions/index.js` use `sanitizeError()` — verified by regex regression test; (3) user-controlled text callables (`moderateContent`, `generateSmartReplies`, `generateListingDescription`) audited: none contain any `console.*` call in their function bodies; (4) SweepNYC debug logs (`signsRaw`, `streetCtx`, `signText`) accepted — values are public NYC Open Data API fields, not user PII; (5) automated regression tests in `utils/redactForLog.test.ts` (utility unit tests + static import assertion + no-raw-message check) and `utils/logRedactionAudit.test.ts` (static console-call audit for coordinates, userData, fcmToken, email); (6) root CI workflow added (`unit-tests.yml`) runs these tests on every push touching `utils/**` or root TS source |
...
| TM-19 | Historical credentials reachable in Git | HIGH historically | Prior audit identified committed provider credentials; status/rotation must be verified through provider metadata without payload access | **PARTIAL REMEDIATION — PROVIDER VERIFICATION PENDING**. Root `.env` tracked in git history across commits `356c61f`–`0dd395f` (all reachable from main); `.env` removed from tracking at `5e29ffeb`. Four credentials inventoried: (1) `VITE_GEMINI_API_KEY` — Gemini API key; no browser restriction at time of commit; usable from any origin; Cloud Functions use Gemini via Secret Manager (not this key); this VITE_ key's activity status UNKNOWN — not confirmed disabled or rotated; **HIGH — ROTATION OR DISABLEMENT REQUIRED BEFORE PUBLIC LAUNCH**; responsible operator: engineering lead. (2) `VITE_GOOGLE_MAPS_API_KEY` — Maps JavaScript API key; Google Maps integration removed from HEAD source (Mapbox GL JS is sole mapping library); **NOT FOUND in current Google Cloud provider credentials as of 2026-07-31 — cannot confirm disablement; provider console verification required before public launch**; safe to disable (no current use in HEAD); disablement is lower risk than rotation. (3) `VITE_MAPBOX_TOKEN` — public `pk.` token (public by design, not a secret); Firebase browser key now has domain restrictions (`parkqueen-46475363-ccf36.web.app/*`, `parkqueen-46475363-ccf36.firebaseapp.com/*`, `localhost:5174/*`); hosted and localhost testing confirmed passing after restriction applied 2026-07-31. (4) `SENDGRID_API_KEY` in `functions/.env` — found only in commit `28385a3` on `spot-details-card-revamp` (local-only branch, never pushed to any remote; branch refs deleted 2026-07-31); **REMEDIATED**: key rotated via Firebase Secret Manager; `generateEmailOTP` redeployed with new secret version bound; OTP sending and verification confirmed working end-to-end; old key revoked. History rewrite deferred — all `.env` commits reachable from main; requires `git filter-repo`, force-push to all remotes, and coordinated contributor re-clone. Required provider actions before public launch: (a) rotate or disable `VITE_GEMINI_API_KEY`; (b) verify and disable `VITE_GOOGLE_MAPS_API_KEY` via Google Cloud Console. |
...
| TM-22 | No Content-Security-Policy | HIGH | `firebase.json` hosting configuration has no `headers` section. `index.html` has no CSP meta tag. All script, style, connect, and media sources are unconstrained. Multiple external CDN domains (cdn.tailwindcss.com, cdnjs.cloudflare.com, esm.sh, api.mapbox.com) are loaded without integrity hashes | **RESOLVED — ENFORCED IN PRODUCTION**. Phase 1 (commit `608cbb5`): security headers + CSP report-only. Phase 2 complete: Tailwind bundled locally (#117), policy flipped to enforced `Content-Security-Policy` with the report-only header removed (#118), `report-uri` to the project's Sentry security endpoint added (#120). FontAwesome is still CDN-hosted from `cdnjs.cloudflare.com` and is explicitly allowed in `style-src`/`font-src` rather than bundled |
