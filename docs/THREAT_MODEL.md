...
| TM-16 | Mutable CI action references | MEDIUM | Checkout/setup actions use version tags; Gitleaks action is SHA-pinned | **Partially fixed** (commit `93aa02b`): `--legacy-peer-deps` removed from firestore-rules.yml (peer deps are resolved); `permissions: contents: read` and `timeout-minutes` added to both workflows; `actions/checkout`, `setup-node`, `setup-java` SHA pinning still requires a manual lookup and commit |
