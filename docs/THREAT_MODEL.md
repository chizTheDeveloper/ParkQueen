...
| TM-04 | Full authenticated user-directory exposure | HIGH | `users/{uid}` read permits every signed-in user; documents include phone/email, FCM token, last geohash/location-related data, vehicle and preference fields | **Partially addressed**: `phone` removed from root doc (TM-24 fix); `email` moved to `users/{uid}/private/account` subcollection; remaining root fields (`displayName`, `username`, `title`, `avatarManifestId`, stats, vehicle) are intentionally public for Ping/chat/leaderboard features. Firestore does not support field-level read restrictions; full isolation requires a `publicProfile` subcollection split and client migration — deferred as separate task |
...
| TM-19 | Historical credentials reachable in Git | HIGH historically | Prior audit identified committed provider credentials; status/rotation must be verified through provider metadata without payload access | **Repository scan complete**: two credential types identified in commit `0dd395f` — `VITE_GOOGLE_MAPS_API_KEY` (browser-restricted Maps key) and `VITE_GEMINI_API_KEY` (Gemini API key). Current secrets confirmed in Firebase Secret Manager only (no plaintext in HEAD). History rewrite deferred (noted in commit `ca8dd88`). Rotation urgency: `VITE_GEMINI_API_KEY` has no browser restriction — HIGH urgency; `VITE_GOOGLE_MAPS_API_KEY` has browser referrer restriction — MEDIUM urgency. **PROVIDER ROTATION VERIFICATION PENDING**: operator must confirm rotation via Google Cloud Console → Credentials → key metadata (lastUsed, status) without accessing the key values. |
