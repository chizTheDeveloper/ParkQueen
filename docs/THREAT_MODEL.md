...
| TM-05 | Incomplete account deletion | HIGH | Callable deletes Auth, root user doc, and username reservation only; linked Pings, feedback, chats/messages, notifications, reports, sessions, avatars, moderation, and trust-event data remain | **Fixed in source** (commit `2d8d0cc`): `deleteAccount` callable now covers all user-linked collections, Storage, private subcollections, and Auth user; uses `db.recursiveDelete()` for subcollection trees; idempotent via `accountDeletionJobs/{uid}` with step tracking; pending production deployment and legal sign-off on audit-log retention |
