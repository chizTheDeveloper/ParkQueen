 ...
 *
 * Confirms retry: true is enabled on the five idempotent system-event
 * handlers proven safe (initUserPrivateAccount, incrementTotalSpotsPinged,
 * updateTrustOnFeedback, updateTrustOnSpotDelete), confirms it remains OFF
 * on awardCrowns (a real transaction-abort poison-event risk — see AC-7 in
 ...
 * redelivered event would retry that exact same throw for the full Eventarc
 * retry window with no escape) and on notifyNearbyUsers (separate messaging
 * retry analysis, not in scope here), and proves duplicate Eventarc delivery
 * is safe for each newly-hardened handler.
 */
...
describe('Eventarc retry configuration — parqueen-system-events handlers', () => {
    ...
    it('initUserPrivateAccount has retry enabled', () => {
