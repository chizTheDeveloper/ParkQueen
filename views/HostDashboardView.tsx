...
export const HostDashboardView = () => {
  ...
  return (
    <div className="pt-20 pb-24 px-4 h-full overflow-y-auto no-scrollbar bg-[var(--color-bg)] text-[var(--color-text)]">
      <div className="flex justify-between items-end mb-6">
        <div>
          <h2 className="text-2xl font-bold">Host Dashboard</h2>
          <p className="text-[var(--color-text-secondary)] text-sm">Welcome back, Queen!</p>
        </div>
        <button 
      ...
      {/* Chart */}
      <div className="bg-[var(--color-surface)] p-4 rounded-xl border border-[var(--color-border)] mb-8 h-72">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[var(--color-surface)] w-full max-w-md rounded-2xl p-6 border border-[var(--color-border)] shadow-2xl">
            <h3 className="text-xl font-bold mb-4">List Your Spot</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-[var(--color-text-secondary)] mb-1">Features (comma separated)</label>
                <input 
                  type="text" 
                  value={newFeatures}
                  onChange={(e) => setNewFeatures(e.target.value)}
                  placeholder="e.g. Gated, CCTV, Wide, Easy Entry"
                  className="w-full bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg p-3 text-[var(--color-text)] focus:border-queen-500 outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg p-3 text-[var(--color-text)] h-24 focus:border-queen-500 outline-none resize-none"
                />
              </div>
              ...
              <div className="flex gap-3 mt-6">
                <button 
                  ...
                  className="flex-1 py-3 text-[var(--color-text-secondary)] font-medium hover:text-[var(--color-text)]"
                >
                  Cancel
                </button>
                <button 
                  onClick={() => setShowAddModal(false)}
                  className="flex-1 bg-white text-[var(--color-bg)] font-bold py-3 rounded-xl hover:bg-queen-100"
                >
                  Create Listing
                </button>
              </div>
